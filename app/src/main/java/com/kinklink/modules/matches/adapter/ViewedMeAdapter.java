...
public class ViewedMeAdapter extends RecyclerView.Adapter<ViewedMeAdapter.ViewHolder> {
    private ArrayList<ViewedMeModel> viewedMeList;
    private Context mContext;
    // variable to track event time
    private long mLastClickTime = 0;

    public ViewedMeAdapter(Context mContext, ArrayList<ViewedMeModel> viewedMeList) {
        this.viewedMeList = viewedMeList;
        this.mContext = mContext;
    }

    @Override
    public ViewedMeAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.views_list, parent, false);
        return new ViewedMeAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(ViewedMeAdapter.ViewHolder holder, int position) {
